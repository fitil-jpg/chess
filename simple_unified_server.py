#!/usr/bin/env python3
"""
Спрощений об'єднаний веб-сервер для Chess AI з дошкою та хітмапами
...
def index():
    ...
    try:
        ...
        logger.warning("Файл unified_chess.html не знайдено, використовуємо простий інтерфейс")
...
@handle_api_errors
def make_bot_move():
    """Зробити хід ботом (простий випадковий бот)"""
    try:
        data = request.get_json() or {}
        bot_name = data.get('bot', 'RandomBot')
        
        # Простий випадковий бот
        moves = list(game_manager.current_board.legal_moves)
        if not moves:
...
@handle_api_errors
def get_stats():
    """Повернути розширені статистики та текстові метрики"""
    try:
        # Phase heuristic
        ...
        # Legal moves count and list preview
        legal = []
        ...
        metrics = [
            f"Фаза гри: {phase}",
            f"Кількість фігур: {len(game_manager.current_board.piece_map())}",
            f"Легальних ходів: {len(legal)}",
            f"Ходів зроблено: {len(game_manager.current_board.move_stack)}"
        ]
...
def generate_heatmap():
    ...
    try:
        ...
        game_phase = data.get('game_phase', 'all')
        piece_type = data.get('piece_type', 'all')
        games_limit = data.get('games_limit', 100)
        
        # Генеруємо тестову теплову карту
        result = generate_mock_heatmap(bot_name, game_phase, piece_type)
...
def get_heatmap_data():
    ...
    try:
        ...
        # Генеруємо тестові дані теплової карти
...
def generate_mock_heatmap(bot_name, game_phase, piece_type):
    """Генерувати тестову теплову карту"""
    import random
    from pathlib import Path
    
    # Створюємо директорію якщо не існує
    output_dir = Path("heatmap_visualizations")
    output_dir.mkdir(exist_ok=True)
    
    # Генеруємо тестові дані
    data = generate_mock_heatmap_data()
    
    # Зберігаємо JSON файл
    json_filename = f"heatmap_{bot_name}_{game_phase}_{piece_type}.json"
...
@handle_api_errors
def get_games():
    """Отримати список ігор (тестові дані)"""
    try:
        # Генеруємо тестові дані
        games = []
        results = ['1-0', '0-1', '1/2-1/2']
        modules = ['StockfishBot', 'DynamicBot', 'RandomBot', 'AggressiveBot', 'FortifyBot']
        ...
        for i in range(10):
            games.append({
                ...
                'movesList': ['e4', 'e5', 'Nf3', 'Nc6', 'Bb5', 'a6', 'Ba4', 'Nf6']
            })
...
@handle_api_errors
def get_modules():
    """Отримати статистику модулів (тестові дані)"""
    try:
        module_stats = {
            'StockfishBot': 25,
            'DynamicBot': 30,
...
def run_server(host='0.0.0.0', port=5000, debug=False):
    ...
    logger.info(f"Запуск спрощеного об'єднаного веб-сервера на {host}:{port}")
    if host in ("0.0.0.0", "::"):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Simple Unified Chess AI Web Server')
